...
impl From<PbNode> for Ipld {
    fn from(pb: PbNode) -> Ipld {
        ...
        map.insert("Data".to_string(), pb.data.into());
        map.into()
    }
}

impl From<PbLink> for Ipld {
    fn from(pb: PbLink) -> Ipld {
        let mut map = BTreeMap::<String, Ipld>::new();
        map.insert("Hash".to_string(), pb.cid.into());
        map.insert("Name".to_string(), pb.name.into());
        map.insert("Tsize".to_string(), pb.size.into());
        map.into()
    }
}
