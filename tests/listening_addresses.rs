...
//
//     let target = libp2p_rs::multiaddr::multiaddr!(Ip4([127, 0, 0, 1]), Tcp(0u16));
//
...
//     let first =
//         node.add_listening_address(libp2p_rs::multiaddr::multiaddr!(Ip4([127, 0, 0, 1]), Tcp(0u16)));
//     let second =
//         node.add_listening_address(libp2p_rs::multiaddr::multiaddr!(Ip4([127, 0, 0, 2]), Tcp(0u16)));
//
//     let (first, second) = futures::future::join(first, second).await;
//
//     // both should succeed
...
//     // perilous.
//     node.add_listening_address(libp2p_rs::multiaddr::multiaddr!(Ip4([0, 0, 0, 0]), Tcp(0u16)))
//         .await
//         .unwrap();
// }
//
...
//     // there is no test in trying to match this with others as ... that would be quite
//     // perilous.
//     let target = libp2p_rs::multiaddr::multiaddr!(Ip4([0, 0, 0, 0]), Tcp(0u16));
//     let first = node.add_listening_address(target.clone());
//     let second = node.add_listening_address(target);
//
//     let (first, second) = futures::future::join(first, second).await;
...
//     let node = ipfs::Node::new("test_node").await;
//
//     let unbound = libp2p_rs::multiaddr::multiaddr!(Ip4([127, 0, 0, 1]), Tcp(0u16));
...
//     use libp2p_rs::multiaddr::Multiaddr;
